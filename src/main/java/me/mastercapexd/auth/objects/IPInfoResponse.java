...
public class IPInfoResponse {
	...
	public String setInfo(String text) {
		if (answer == IPInfoAnswer.ERROR)
			return text.replaceAll("(?i)%country_code%", "NONE").replaceAll("(?i)%town%", "NONE")
					.replaceAll("(?i)%flag_emoji%", "🏳");
		if (answer == IPInfoAnswer.SUCCESS)
			return text.replaceAll("(?i)%country_code%", countryCode).replaceAll("(?i)%town%", townName)
					.replaceAll("(?i)%flag_emoji%", flagEmoji);
		return text;
	}
