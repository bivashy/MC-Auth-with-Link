...
public class MessengerAuthenticationStep extends AbstractAuthenticationStep {
	...
	public MessengerAuthenticationStep(String stepName, AuthenticationStepContext authenticationStepContext,
			LinkEntryUser linkEntryUser) {
	...
	public boolean shouldSkip() {
		...
		if (linkUser == null) {
			linkType.getProxyMessages().getMessage("not-linked").ifPresent(component -> linkEntryUser.getAccount()
					.getPlayer().get().sendMessage(linkType.newMessageContext(account).apply(component.legacyText())));
			return true;
		}
		...
		Keyboard keyboard = linkType.getSettings().getKeyboards().createKeyboard("confirmation", "%name%",
				account.getName());

		Identificator userIdentificator = linkUserInfo.getIdentificator().isNumber()
				? Identificator.of(linkUserInfo.getIdentificator().asNumber())
				: Identificator.of(linkUserInfo.getIdentificator().asString());
		linkType.newMessageBuilder(
				linkType.getSettings().getMessages().getMessage("enter-message", linkType.newMessageContext(account)))
				.keyboard(keyboard).build().send(userIdentificator);
		return false;
	}
