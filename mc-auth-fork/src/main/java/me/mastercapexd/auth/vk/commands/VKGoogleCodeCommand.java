	...
	public void execute(VKMessageEvent e, String[] args) {
		...
		if (args.length < 2) {
			sendMessage(e.getPeer(),
					receptioner.getConfig().getVKSettings().getVKMessages().getMessage("google-code-not-enough-arguments"));
			return;
		}
		String playerName = args[0];
		if (!isInteger(args[1]) || args[1].length() != 6) {
			sendMessage(e.getPeer(), receptioner.getConfig().getVKSettings().getVKMessages().getMessage("google-need-integer"));
			return;
		}
		...
		receptioner.actionWithAccount(e.getUserId(), playerName, account -> {
			...
			if (!Auth.hasGoogleAuthAccount(account.getId())) {
				sendMessage(e.getPeer(),
						receptioner.getConfig().getVKSettings().getVKMessages().getMessage("google-code-account-not-need-enter"));
				return;
			}
