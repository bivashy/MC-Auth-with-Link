	...
	private Function<Collection<Account>, MessagesSendQuery> getAccountsConsumer() {
		return (findedAccounts -> {
			...
			if (findedAccounts.isEmpty()) {
				sendQuery.message(config.getVKSettings().getMessages().getMessage(type.getNoAccountsPath()));
				return sendQuery;
			}
			...
			sendQuery.keyboard(keyboard)
					.message(config.getVKSettings().getMessages().getMessage(type.getAccountsPath()));
			return sendQuery;
		});
	}
