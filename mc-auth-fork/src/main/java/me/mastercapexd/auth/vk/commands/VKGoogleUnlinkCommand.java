	...
	public void execute(VKMessageEvent e, String[] args) {
		...
		if (args.length == 0) {
			sendMessage(e.getPeer(),
					receptioner.getConfig().getVKMessages().getLegacyMessage("google-unlink-not-enough-arguments"));
			return;
		}
		...
		receptioner.actionWithAccount(e.getUserId(), playerName, account -> {
			if (account.getGoogleKey() == null || account.getGoogleKey().isEmpty()) {
				sendMessage(e.getPeer(),
						receptioner.getConfig().getVKMessages().getLegacyMessage("google-unlink-not-have-google"));
				return;
			}
			account.setGoogleKey(null);
			receptioner.getAccountStorage().saveOrUpdateAccount(account);
			sendMessage(e.getPeer(), receptioner.getConfig().getVKMessages().getLegacyMessage("google-unlinked"));
		});
	}
