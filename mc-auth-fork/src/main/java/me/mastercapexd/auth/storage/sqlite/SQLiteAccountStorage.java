...
public class SQLiteAccountStorage extends SQLAccountStorage {
	private String url;
	private static final String CREATE_TABLE = "CREATE TABLE IF NOT EXISTS `auth` (`id` VARCHAR(50) PRIMARY KEY, `uuid` VARCHAR(64) NOT NULL, `name` VARCHAR(32) NOT NULL, `password` VARCHAR(255), `vkId` INTEGER NOT NULL, `last_quit` BIGINT, `last_ip` VARCHAR(64), `last_session_start` INTEGER, `id_type` VARCHAR(32) NOT NULL, `hash_type` VARCHAR(32) NOT NULL);";
	...
	private static final String UPDATE_ID = "REPLACE INTO `auth` (`id`, `uuid`, `name`, `password`, `vkId`, `last_quit`,  `last_ip`, `last_session_start`, `id_type`, `hash_type`) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?,?);";
