...
public class MySQLAccountStorage extends SQLAccountStorage {
    private static final String CREATE_TABLE = "CREATE TABLE IF NOT EXISTS `auth` (`id` VARCHAR(64) PRIMARY KEY, `uuid` VARCHAR(64) NOT NULL, `name` VARCHAR" +
            "(32) NOT NULL, `password` VARCHAR(255),`google_key` VARCHAR(64), `vkId` INT NOT NULL,`vk_confirmation_enabled` BIT,`telegram_id` BIGINT NOT " +
            "NULL,`telegram_confirmation_enabled` BIT, `last_quit` BIGINT, `last_ip` VARCHAR(64), `last_session_start` BIGINT, `id_type` VARCHAR(32) NOT " +
    ...
    private static final String UPDATE_ID = "INSERT INTO `auth` (`id`, `uuid`, `name`, `password`,`google_key`,`vkId`,`vk_confirmation_enabled`," +
            "`telegram_id`,`telegram_confirmation_enabled`, `last_quit`, `last_ip`, `last_session_start`, `id_type`, `hash_type`) VALUES " + "(?, ?, ?, ?, ?," +
            " ?, ?, ?, ?, ?, ?, ?, ?, ?) ON DUPLICATE KEY UPDATE " + "`id` = VALUES(`id`), `uuid` = VALUES(`uuid`), `name` = VALUES(`name`), " + "`password` " +
            "= VALUES(`password`),`google_key` = VALUES(`google_key`),`vkId` = VALUES(`vkId`),`vk_confirmation_enabled` = VALUES(`vk_confirmation_enabled`)," +
            "`telegram_id` = VALUES(`telegram_id`),`telegram_confirmation_enabled` = VALUES(`telegram_confirmation_enabled`), `last_quit` = VALUES" +
            "(`last_quit`), `last_ip` = VALUES(`last_ip`), `last_session_start` = VALUES(`last_session_start`), `id_type` = VALUES(`id_type`), `hash_type` = " +
            "VALUES(`hash_type`);";
